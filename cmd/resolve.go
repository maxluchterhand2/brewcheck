...
func resolveTarget(ctx context.Context, positional string, cfg config) (*resolved, error) {
	if cfg.tap != "" {
		return resolveTap(ctx, cfg.tap, positional, cfg)
	}
	...
	switch {
	case cfg.formula != "":
		return resolveFormula(ctx, client, cfg.formula, cfg)
	case cfg.cask != "":
		return resolveCask(ctx, client, cfg.cask)
	case positional != "":
		return resolvePositional(ctx, client, positional, cfg)
	default:
		return nil, errors.New("provide a name: brewcheck <name> | --formula <name> | --cask <name>")
	}
}

func resolvePositional(ctx context.Context, client *api.Client, name string, cfg config) (*resolved, error) {
	...
	case formulaOK:
		return formulaTarget(f.Name, f, cfg)
	case caskOK:
		return buildCask(k)
	default:
...
func resolveFormula(ctx context.Context, client *api.Client, name string, cfg config) (*resolved, error) {
...
func formulaTarget(name string, f *api.Formula, cfg config) (*resolved, error) {
	platform, err := api.HostPlatform()
	if err != nil {
		return nil, err
	}

	if !cfg.buildFromSource {
		if key, bottle, err := f.SelectBottle(platform); err == nil {
			return &resolved{
				name:          name,
				kind:          report.KindFormula,
	...
	return &resolved{
		...
		githubRepo:    f.GitHubRepo(),
		fromSource:    true,
		fetcher:       download.NewHTTPFetcher(src),
	}, nil
}
